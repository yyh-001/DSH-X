...
- **整合包**：一次装好一批插件和它们的配置（生态里的 `.dspack`：本地文件、直链、GitHub 仓库，或内置的社区市场）。装之前先摆清「装什么、装到哪、会覆盖什么」，默认装进独立 profile 与现有环境互不打扰；安装前备份、失败回滚、卸载还原。也能把当前 profile 导出成整合包发人
- **兼容模式**：启动失败按报错自动禁用出问题的插件（可一键恢复）；启动后自检页面引用的客户端插件包，管理页给出结论（区分实例问题和旧标签页）
- **同步 / 导出即备份**：会话记录、附件和插件配置传到 S3 兼容的存储桶或 WebDAV 目录（坚果云 / Nextcloud / 群晖 / Alist…），换机器时拉回来；也可以导出到一个本地文件夹再导回来（不联网、不要账号）。插件清单两个方向都合并，不会丢掉任何一边装的插件
